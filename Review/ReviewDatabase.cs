...
namespace Review
{
	public class ReviewDatabase
	{
		...
		{
			var connection =
				new NpgsqlConnection("Server=kelite-yt-board;Port=5432;User Id=postgres;Password=kelite123;Database=review;");
			connection.Open();
			return connection;
		}
			...
			using (NpgsqlConnection connection = OpenConnection())
			{
				using (
					var command = new NpgsqlCommand(
						"select commit_hash, status, comment from review where commit_hash = :commit_hash", connection))
				{
		...
		public void Save(ReviewInfo reviewInfo)
		{
			...
			using (NpgsqlConnection connection = OpenConnection())
			{
				using (NpgsqlTransaction transaction = connection.BeginTransaction())
				{
					long count;
					using (
						var countCommand = new NpgsqlCommand("select count(*) from review where commit_hash = :commit_hash", connection,
							transaction))
					{
					...
					if (count == 0)
					{
						using (
							var countCommand =
								new NpgsqlCommand("insert into review (commit_hash, status, comment) VALUES (:commit_hash, :status, :comment)",
					...
					else
					{
						using (
							var countCommand =
								new NpgsqlCommand("update review set status=:status, comment=:comment where commit_hash=:commit_hash",
									connection, transaction))
						{
							countCommand.Parameters.Add(new NpgsqlParameter("commit_hash", commitHash));
							countCommand.Parameters.Add(new NpgsqlParameter("status", status));
